...
//                       XA      YA      ZA      XG      YG      ZG
int MPUOffsets[6] = { 1142,     603,    1122,       7,     162,     -35
}; //MPU6050 on balanceing bot
...
void GetDMP() { // Best version I have made so far
  ...
  */
  ...
  // FJ: FIFO overflow OR incomplete FIFO sample.
  if ((fifoCount % packetSize) || (fifoCount > (packetSize * MaxPackets))) { // we have failed Reset and wait till next time!
