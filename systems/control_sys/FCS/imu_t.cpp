...
int IMU::setup(bool calib)
{
    ...
    power_up();
    set_DLPF(DLPF_CFG); // 44 Hz
    // set_range(0, 0); // Always set to ±250˚/s, ±2g
    // set_sample_rate(0); // set sample rate to 8k?
    if(calib) {
        calibrate();
    }
    return 0;
...
void IMU::calc_ypr()
{
    ...
    for (int i=0; i<2; ++i) {
        ...
        // Serial.println(raw_data[i+4] - offset.raw[i+4]);
        // float acc_th = (atan2(raw_data[i], raw_data[2]) - offset.acc_pr[i]) * RTD_CCF; // RAD_TO_DEG * (1-COMP_COEF) 
        float acc_th = atan2(raw_data[i], raw_data[2]) * RTD_CCF; // RAD_TO_DEG * (1-COMP_COEF) 
        if (i == 0) acc_th *= -1;
...
void IMU::read_raw()
{
    uint8_t temp_buf[14] = {0}; // buffer to hold all data, 14 bytes
    ...
    */ 
    ...
    raw_data[1] = (temp_buf[2])<<8 | temp_buf[3];
...
void IMU::calibrate()
{
    // delay(1000); // To stabilise IMU? not sure if needed
    ...
    for (int i=0; i<6; ++i) {
        ...
        // Serial.print(offset.raw[i]);
        // Serial.print(" | ");
    }
    
    offset.acc_pr[0] = atan2(offset.raw[0], offset.raw[2]);
    offset.acc_pr[1] = atan2(offset.raw[1], offset.raw[2]);
    // Serial.print(offset.acc_pr[0]); Serial.print(" | ");
    // Serial.print(offset.acc_pr[1]); Serial.println();
    /*
...
void IMU::set_DLPF(uint8_t cfg)
{
    uint8_t cur_val =  read_reg(CONFIG) | 0b0000111;
    cur_val = cur_val & cfg;
    write_reg(CONFIG, cur_val);
}
...
float * IMU::get_acc_offset_ang()
{
    return offset.acc_pr;
}

int32_t * IMU::get_offset_raw()
{
    return offset.raw;
}
